...
class ThreeDRebuilder:
    ...
    def get_min_coords(self):
        ...
        self.left_model=self.result.get("left_model")
    def draw_model(self):
        """绘制 3D 模型"""
    ...
    def run(self):
        ...
        self.get_min_coords()
        lines, arcs, circles = self.draw_model()
        ...
        if self.cluster_max_y_t is not None and self.cluster_min_y_f is not None:
            ...
            # 可根据业务需要设置默认值或抛出异常
            height_diff = 0  
        return lines, arcs, circles,height_diff,self.left_model